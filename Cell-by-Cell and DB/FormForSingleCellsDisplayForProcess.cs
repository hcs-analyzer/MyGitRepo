...
namespace HCSAnalyzer.Forms.FormsForGraphsDisplay
{
    public partial class FormForSingleCellsDisplay : Form
    {
        ...
        cExtendedTable ListZfactors = null;
...
#if BUILD64
            this.chartForPoints.ChartAreas[0].CursorX.Interval = (long)double.Epsilon;
            this.chartForPoints.ChartAreas[0].CursorY.Interval = (long)double.Epsilon;
#else
        ...
        ContextMenuStrip buildMenu(MouseEventArgs e)
        {
            ...
            AnalysisContextMenu.DropDownItems.Add(new ToolStripSeparator());

            ToolStripMenuItem ToolStripMenuItem_AnalysisZfactor = new ToolStripMenuItem("Z Factor");
            ToolStripMenuItem_AnalysisZfactor.Click += new System.EventHandler(this.ToolStripMenuItem_AnalysisZfactor);
            AnalysisContextMenu.DropDownItems.Add(ToolStripMenuItem_AnalysisZfactor);
            ...
            {
                ToolStripMenuItem ToolStripMenuItem_DisplayImages = new ToolStripMenuItem("Display Associated Images");
                ToolStripMenuItem_DisplayImages.Click += new System.EventHandler(this.ToolStripMenuItem_DisplayImages);
                NewMenu.Items.Add(ToolStripMenuItem_DisplayImages);
            }
        ...
        private void ToolStripMenuItem_DisplayImages(object sender, EventArgs e)
        {
            ...
            // check the number of point about to be processed a give a warning if necessary
            int IDxPt = 0;
            int PosXImage = 0;
            ...
            if (IDxPt > 30)
            {
                if (MessageBox.Show(IDxPt + " objects have been selected.\nDo you want continue?", "Warning !", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == System.Windows.Forms.DialogResult.No)
                    return;
            ...
            cListGeometric3DObject ListObjects = new cListGeometric3DObject("Image Phenotypes MetaObject");
            ...
            foreach (DataPoint item in this.chartForPoints.Series[0].Points)
            {
                if ((item.XValue >= MinX) && (item.XValue <= MaxX) && (item.YValues[0] >= MinY) && (item.YValues[0] <= MaxY))
                {
                    // if ((item.Tag != null) && (item.Tag.GetType() == typeof(cSingleBiologicalObject)))
                    {
                        ...
                        // extract the image
                        List<cImageMetaInfo> ListInfo = cGlobalInfo.ImageAccessor.GetImageInfo(TmpObject);
                        cImage Image = new cImage(ListInfo);
                        TmpObject.BD_BoxMax.Z = TmpObject.BD_BoxMin.Z = 0;

                        //     TmpObject.BD_BoxMin.Y = Image.Height - TmpObject.BD_BoxMin.Y;
                        //    TmpObject.BD_BoxMax.Y = Image.Height - TmpObject.BD_BoxMax.Y;

                        if (TmpObject.BD_BoxMin.Y > TmpObject.BD_BoxMax.Y)
                        {
                            double Tmp = TmpObject.BD_BoxMin.Y;
                            TmpObject.BD_BoxMin.Y = TmpObject.BD_BoxMax.Y;
                            TmpObject.BD_BoxMax.Y = Tmp;
                        }

                        cPoint3D TmpMin = new cPoint3D(TmpObject.BD_BoxMin);
                        cPoint3D TmpMax = new cPoint3D(TmpObject.BD_BoxMax);
                        ...
                        // CroppedImaged.GetBitmap(1,

                        //   CroppedImaged.AssociatedImagePanel.LUTManager.PastFromClipBoard();

                        // cImage CroppedImaged = Image.Crop(TmpObject.BD_BoxMin, TmpObject.BD_BoxMax);

                        //  cDisplaySingleImage DSI = new cDisplaySingleImage();
                        //  DSI.SetInputData(I);
                        //  DSI.Run();

                        c3DTexturedPlan ImageTexture = new c3DTexturedPlan(new cPoint3D(PosXImage, 0, 0), CroppedImaged);

                        PosXImage += (CroppedImaged.Width);
                        ...
                        ListObjects.AddObject(ImageTexture);
                        ListObjects[ListObjects.Count - 1].Tag = TmpObject;
                        //cDisplaySingleImage IV = new cDisplaySingleImage();
                        //IV.IsUseSavedDefaultDisplayProperties = true;
                        //IV.DefaultZoom = 100;
                        //IV.Title = "[Plate] " + TmpObject.AssociatedWell.AssociatedPlate.GetName() + " - [Well] " + TmpObject.AssociatedWell.GetPos() + " - [Field] " + TmpObject.ImageField + " - " + TmpObject.CellularPhenotypeType.Name;
            ...
            MyWorld.AddGeometric3DObjects(ListObjects);
            ...
            CDW.Title = "Images " + ListObjects.Count + " objects";
...
#if BUILD64
            this.chartForPoints.ChartAreas[0].CursorX.Interval = (long)double.Epsilon;// (ListX.Max() - ListX.Min()) / 1000.0;
            this.chartForPoints.ChartAreas[0].CursorY.Interval = (long)double.Epsilon;// (ListY.Max() - ListY.Min()) / 1000.0;
#else
        ...
        private void buttonStartCluster_Click(object sender, EventArgs e)
        {
            ...
            if (MachineLearning.SelectedClusterer.GetNumberOfClusters() > cGlobalInfo.ListCellularPhenotypes.Count)
            {
                MessageBox.Show("Number of identifed clusters (" + MachineLearning.SelectedClusterer.GetNumberOfClusters() + ") not handled by the application !", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                this.Cursor = Cursors.Default;
                return;
            }
            ...
            if (MachineLearning.SelectedClusterer != null)
            {
                double[] Assign = MachineLearning.EvaluteAndDisplayClusterer(this.richTextBoxForResults,
                                        ...
                                        this.InputTable.CreateWekaInstances()).getClusterAssignments();
        ...
        private void ToolStripMenuItem_AnalysisZfactor(object sender, EventArgs e)
        {
            ComputeZfactor();
        }
        ...
        void ComputeZfactor()
        {
            ...
            int IdxClass0 = cGlobalInfo.CtrlNeg.GetIdx();
            int IdxClass1 = cGlobalInfo.CtrlPos.GetIdx();


            string SelectedDesc = this.comboBoxAxeX.Text;
            int IdxDesc = this.comboBoxAxeX.SelectedIndex;

            cExtendedTable ListValuesClass0 = new cExtendedTable();
            cExtendedTable ListValuesClass1 = new cExtendedTable();

            for (int j = 0; j < this.chartForPoints.Series[0].Points.Count; j++)
            {
                if (this.ListPhenotypeVisible[(int)MachineLearning.Classes[j]])
                {
                    cSingleBiologicalObject SBO = (cSingleBiologicalObject)this.InputTable.ListTags[j];

                    if (SBO.AssociatedWell.GetClassType() == cGlobalInfo.ListWellClasses[IdxClass0])
                    {
                        if (ListValuesClass0.Count == 0)
                        {
                            ListValuesClass0.Add(new cExtendedList());
                            ListValuesClass0[0].Tag = SBO.AssociatedWell;
                            ListValuesClass0[0].Add(this.InputTable[IdxDesc][j]);
                        }
                        else
                        {
                            bool IsAdded = false;
                            for (int k = 0; k < ListValuesClass0.Count; k++)
                            {
                                if (SBO.AssociatedWell == (cWell)ListValuesClass0[k].Tag)
                                {
                                    ListValuesClass0[k].Add(this.InputTable[IdxDesc][j]);
                                    IsAdded = true;
                                    break;
                                }
                            }
                            ...
                            {
                                ListValuesClass0.Add(new cExtendedList());
                                ListValuesClass0[ListValuesClass0.Count - 1].Tag = SBO.AssociatedWell;
                                ListValuesClass0[ListValuesClass0.Count - 1].Add(this.InputTable[IdxDesc][j]);
                            }
                        }

                    }
                    else if (SBO.AssociatedWell.GetClassType() == cGlobalInfo.ListWellClasses[IdxClass1])
                    {
                        if (ListValuesClass1.Count == 0)
                        {
                            ListValuesClass1.Add(new cExtendedList());
                            ListValuesClass1[0].Tag = SBO.AssociatedWell;
                            ListValuesClass1[0].Add(this.InputTable[IdxDesc][j]);
                        }
                        else
                        {
                            bool IsAdded = false;
                            for (int k = 0; k < ListValuesClass1.Count; k++)
                            {
                                if (SBO.AssociatedWell == (cWell)ListValuesClass1[k].Tag)
                                {
                                    ListValuesClass1[k].Add(this.InputTable[IdxDesc][j]);
                                    IsAdded = true;
                                    break;
                                }
                            }
                            ...
                            {
                                ListValuesClass1.Add(new cExtendedList());
                                ListValuesClass1[ListValuesClass1.Count - 1].Tag = SBO.AssociatedWell;
                                ListValuesClass1[ListValuesClass1.Count - 1].Add(this.InputTable[IdxDesc][j]);
                            }
                        }
                    }
                }
            ...
            string ShortInfoZFactors = SelectedDesc + "\n" + cGlobalInfo.CtrlNeg.Name + " vs. " + cGlobalInfo.CtrlPos.Name;

            richTextBoxForResults.AppendText("\n--------------------------------------\nZ-Factor\n--------------------------------------\n\n");
            richTextBoxForResults.AppendText("Descriptor: [" + SelectedDesc + "]\n\n");
            richTextBoxForResults.AppendText("Control 0: [" + cGlobalInfo.CtrlNeg.Name + "]\n");
            string TMpString = "";
            for (int i = 0; i < ListValuesClass0.Count; i++)
                TMpString += "\tWell " + i + " - " + ListValuesClass0[i].Count + " object(s)\n";
            TMpString += "\n";
            richTextBoxForResults.AppendText(TMpString);


            richTextBoxForResults.AppendText("Control 1: [" + cGlobalInfo.CtrlPos.Name + "]\n");
            TMpString = "";
            for (int i = 0; i < ListValuesClass1.Count; i++)
                TMpString += "\tWell " + i + " - " + ListValuesClass1[i].Count + " object(s)\n";
            TMpString += "\n";
            richTextBoxForResults.AppendText(TMpString);


            // average per cell population
            cStatistics CS = new cStatistics();
            CS.UnselectAll();
            CS.IsMean = true;
            CS.SetInputData(ListValuesClass1);
            CS.Run();
            ListValuesClass1 = CS.GetOutPut();
            CS.SetInputData(ListValuesClass0);
            CS.Run();
            ListValuesClass0 = CS.GetOutPut();

            // transpose matrix
            cTranspose T = new cTranspose();
            T.SetInputData(ListValuesClass0);
            T.Run();
            ListValuesClass0 = T.GetOutPut();
            T.SetInputData(ListValuesClass1);
            T.Run();
            ListValuesClass1 = T.GetOutPut();

            // Merge results
            cMerge M = new cMerge();
            M.IsHorizontal = true;
            M.SetInputData(ListValuesClass0, ListValuesClass1);
            M.Run();
            ListValuesClass0 = M.GetOutPut();

            if ((ListValuesClass0 == null) || (ListValuesClass1 == null))
            {
                TMpString = "Z Factor = NaN\n";
                richTextBoxForResults.AppendText(TMpString);
                return;

            }

            ListValuesClass0[0].Name = cGlobalInfo.CtrlNeg.Name;
            ListValuesClass0[1].Name = cGlobalInfo.CtrlPos.Name;

            cZFactor ZF = new cZFactor();
            ZF.SetInputData(ListValuesClass0);
            ZF.Run();

            double ZFactor = ZF.GetOutPut()[0][1];

            TMpString = "Z Factor = " + ZFactor + "\n";
            richTextBoxForResults.AppendText(TMpString);

            if (this.ListZfactors == null)
            {
                this.ListZfactors = new cExtendedTable();
                this.ListZfactors.Add(new cExtendedList("Z Factors"));
                this.ListZfactors.ListRowNames = new List<string>();
            }
            this.ListZfactors.ListRowNames.Add(ShortInfoZFactors);
            this.ListZfactors[0].Add(ZFactor);
        ...
        private void processToolStripMenuItem_DropDownOpening(object sender, EventArgs e)
        {
            ...
            while (TSIC.Count != 0)
            {
                processToolStripMenuItem.DropDownItems.Add(TSIC[0]);
            }
        }

        private void computeZFactorToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ComputeZfactor();
        }
        ...
        {
            cDisplayExtendedTable DET = new cDisplayExtendedTable();
            this.ListZfactors.Name = "info";
            DET.SetInputData(this.ListZfactors);
            DET.Title = "Z factors";
            DET.Run();
        }
        ...
        {
            if(MessageBox.Show("Performing this operation will erase the current list!\nDo you want continue?", "Warning", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation)!= System.Windows.Forms.DialogResult.Yes) return;

            //this.ListZfactors.Add(new cExtendedList());
            this.ListZfactors[0].Clear();
            this.ListZfactors.ListRowNames.Clear();
        }

        private void analysisToolStripMenuItem_Click(object sender, EventArgs e)
        {
            FormForHeterogeneityAnalysis FFHA = new FormForHeterogeneityAnalysis(this);
