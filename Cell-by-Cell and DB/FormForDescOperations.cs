...
namespace HCSAnalyzer.Cell_by_Cell_and_DB
{
    public partial class FormForDescOperations : Form
    {
        ...
        void UpdateDescName()
        {
            ...
            else
            {
                if (radioButtonSQRT.Checked)
                    ...
                    this.textBoxNewDescName.Text = "Sin(" + comboBoxDescriptor1.Text + ")";
                else if (radioButtonCosine.Checked)
                    this.textBoxNewDescName.Text = "Cos(" + comboBoxDescriptor1.Text + ")";
        ...
        {
            richTextBoxInfo.Clear();
            richTextBoxInfo.AppendText("Return the sine of the data.");
            UpdateDescName();
        }

        private void radioButtonCosine_CheckedChanged(object sender, EventArgs e)
        {
            richTextBoxInfo.Clear();
            richTextBoxInfo.AppendText("Return the cosine of the data.");
            UpdateDescName();
