    ...
    public partial class HCSAnalyzer : Form
    {
        ...
        public class cConvertCSVtoDB
        {
            ...
            private void backgroundWorkerForCSVtoDB_DoWork(object sender, DoWorkEventArgs e)
            {
                ...
                do
                {
                    // => extract or generate the plate name
                    #region plate name extraction
                    if (ColPlateName == -1)
                    {
                    ...
                    do
                    {
                        ...
                        cWellForDatabase WellForDB = null;
                        if (!ListPlatesToBeExcluded.Contains(ConvertedName))
                        {
                            WellForDB = new cWellForDatabase(OriginalPlatePlateName, Pos[0], Pos[1]);
                        }
                        ...
                        do
                        {
                            ...
                            for (int idxDesc = 0; idxDesc < ColsForDescriptors.Length; idxDesc++)
                            {
                                ...
                                if (double.TryParse(CurrentDesc[ColsForDescriptors[idxDesc]], NumberStyles.Any, CultureInfo.InvariantCulture, out Value))
                                {
                            ...
                            // if not, put it at 0
                            double ValueClass;
                            string CurrentPhenoName = "";
                            if (ColPhenotypeClass != -1)
                            {
                                CurrentPhenoName = CurrentDesc[ColPhenotypeClass];
                            }

                            if ((ColPhenotypeClass != -1) && (double.TryParse(CurrentPhenoName, out ValueClass) == true))
                            {
                            ...
                            else
                            {
                                ...
                                if (IsAdded == false)  // time to add a new phenotype
                                {
                                    ...
                                    Signature.Add(ListPhenotypeNames.Count);
                                    ListPhenotypeNames.Add(CurrentPhenoName);
                                }
                                //Signature.Add(0);
                            }
                            ...
                            else
                            {
                                ...
                                foreach (var c in System.IO.Path.GetInvalidFileNameChars())
                                    ConvertedName = CurrentPlateName.Replace(c, '-');
                            }
                        ...
                        {
                            this.CompleteReportString += "\t" + CurrentWellPos + " : skipped\n";
                        }
        ...
        public void LoadDB(string DefaultPath, bool IsDisplayPlateSelection)
        {
            ...
            FormForPlateDimensions PlateDim = new FormForPlateDimensions();
            PlateDim.checkBoxAddCellNumber.Checked = true;
            PlateDim.checkBoxAddCellNumber.Visible = true;
            PlateDim.checkBoxIsOmitFirstColumn.Visible = true;
            PlateDim.labelHisto.Visible = true;
            PlateDim.numericUpDownHistoSize.Visible = true;
            ...
            string PathForOption = ListFilesOption[0];

            if (ListFilesOption.Length > 1)
            {
                FormForPlateSelection FFP = new FormForPlateSelection(ListFilesOption, false);
                FFP.Text = "Option File";

                if (FFP.ShowDialog() != System.Windows.Forms.DialogResult.OK) return;
                PathForOption = FFP.GetListPlatesSelected()[0];
            }
        ...
        private void simpleTestToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            //MyProcess.StartInfo.Arguments = "I:\\CardiacSpheroids\\5 min 120615\\B02\\results\\post\\Original_B02_post.avi";
        ...
        private void HCSAnalyzer_DragDrop(object sender, DragEventArgs e)
        {
            if (e.Data.GetDataPresent(DataFormats.FileDrop))
            {
                ...
                string FileType = files[0].Remove(0, files[0].Length - 4);
        ...
        private void HCSAnalyzer_DragEnter(object sender, DragEventArgs e)
        {
            if (e.Data.GetDataPresent(DataFormats.FileDrop))
        ...
        private void columbusDBToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ColumbusToDB("");
        }

        private void ColumbusToDB(string DefaultPath)
        {
            #region Get the directory
            if (!Directory.Exists(DefaultPath)) DefaultPath = "";
            ...
            try
            {
                ListFilesForPlates = Directory.GetFiles(Path, "*Population - *.txt", SearchOption.AllDirectories);
            }
            catch (System.Exception excep)
            {
                MessageBox.Show(excep.Message, "Error!", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            foreach (var item in ListFilesForPlates)
            {
                ...
                CurrentName = CurrentName.Remove(CurrentName.Length - 4);
                Sep[0] = "-";
                Res = CurrentName.Split(Sep, StringSplitOptions.RemoveEmptyEntries);
                Sep[0] = " ";

                char[] TrimChar = new char[1];
                TrimChar[0] = ' ';
                CurrentName = Res[Res.Length - 1].TrimStart(TrimChar);
                Sep[0] = "[";
                Res = CurrentName.Split(Sep, StringSplitOptions.RemoveEmptyEntries);
                CurrentName = Res[0];

                if (CurrentPlateDico.ContainsKey(CurrentName)) continue;

                CurrentPlateDico.Add(CurrentName, item);
            ...
            for (int TableIdx = 0; TableIdx < ListFilesForPlates.Length; TableIdx++)
            {
                MyProgressBar.progressBar.Value = TableIdx;
                MyProgressBar.richTextBoxForComment.AppendText(ListFilesForPlates[TableIdx]);
                ...
                CSVTable.Separator = "\t";
                CSVTable.SetInputData(ListFilesForPlates[TableIdx]);
                CSVTable.Run();

                cExtendedTable TmpTable = CSVTable.GetOutPut();

                if (TmpTable.Count == 0) continue;

                // replace the plate name by the current top directory
                string[] ListS = ListFilesForPlates[TableIdx].Split(Sep, StringSplitOptions.RemoveEmptyEntries);
                string PlateName = ListS[ListS.Length - 2];

                for (int i = 0; i < TmpTable[2].ListTags.Count; i++)
                {
                    TmpTable[2].ListTags[i] = PlateName;
                }
            ...
            {
                if (FinalTable[i].Name == "Bounding Box")
                {
                    cExtendedList MinX = new cExtendedList("MinX");
                    cExtendedList MinY = new cExtendedList("MinY");
                    cExtendedList MaxX = new cExtendedList("MaxX");
                    cExtendedList MaxY = new cExtendedList("MaxY");
                    MinX.ListTags = new List<object>();
                    MinY.ListTags = new List<object>();
                    MaxX.ListTags = new List<object>();
                    MaxY.ListTags = new List<object>();

                    char[] TrimC = new char[1];

                    for (int j = 0; j < FinalTable[i].ListTags.Count; j++)
                    {
                        string TmpStr = FinalTable[i].ListTags[j].ToString();
                        TrimC[0] = '[';
                        TmpStr = TmpStr.TrimStart(TrimC);
                        TrimC[0] = ']';
                        TmpStr = TmpStr.TrimEnd(TrimC);
                        TrimC[0] = ',';
                        string[] Splitted = TmpStr.Split(TrimC);
                        MinX.ListTags.Add(Splitted[0]);
                        MinY.ListTags.Add(Splitted[1]);
                        MaxX.ListTags.Add(Splitted[2]);
                        MaxY.ListTags.Add(Splitted[3]);

                    }

                    FinalTable.RemoveAt(i);
                    FinalTable.Add(MinX);
                    FinalTable.Add(MaxX);
                    FinalTable.Add(MinY);
                    FinalTable.Add(MaxY);
            ...
            LS.Add("ScreenID");
            LS.Add("PlateID");
            LS.Add("MeasurementDate");
            LS.Add("MeasurementID");

            FinalTable.RemoveByName(LS);

            // create the CVS file
            string TPath = Path + "\\" + FFP.GetNamesSelected()[0] + ".csv";

            cTableToFile TTF = new cTableToFile();
            TTF.Separator = ",";
            TTF.SetInputData(FinalTable);
            TTF.FileType = eFileType.CSV;
            TTF.FilePath = TPath;
        ...
        private void fACSToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        private void loadTiffToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        public void LoadTIFF(string filename)
        {
            ...
            fs.Close();
            // check if little indian. If not then end....
            if (data[0] != 73) return;

            int ID = (data[2] << 0) + (data[3] << 8);
            ...
            // jmp to to the start
            // extract the header
            long Shift = InitShift;

            int NumberofTags = (data[Shift++] << 0) + (data[Shift++] << 8);
            List<long> ValuesTag = new List<long>();
            ...
            string Software = "";
            string DateTime = "";
            // each tag is made of 12 bytes
            for (int i = 0; i < NumberofTags; i++)
            {

                // 256 :  ImageWidth

                int TagId = (data[Shift++] << 0) + (data[Shift++] << 8);
                ...
                int DataType = (data[Shift++] << 0) + (data[Shift++] << 8);

                long DataCount = (data[Shift++] << 0) + (data[Shift++] << 8) + (data[Shift++] << 16) + (data[Shift++] << 32);
            ...
            if (BitsPerSample == 8)
            {
                for (int Y = 0; Y < Im.Height; Y++)
                    for (int X = 0; X < Im.Width; X++)
                        Im.SingleChannelImage[0].Data[X + Y * Im.Width] = data[Shift++];
            ...
            else if (BitsPerSample == 16)
            {
                for (int Y = 0; Y < Im.Height; Y++)
                    for (int X = 0; X < Im.Width; X++)
                        Im.SingleChannelImage[0].Data[X + Y * Im.Width] = (data[Shift++] << 0) + (data[Shift++] << 8);
            }
