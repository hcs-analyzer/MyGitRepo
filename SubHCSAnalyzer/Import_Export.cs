    ...
    public partial class HCSAnalyzer
    {
        ...
        private void importToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            //  if (CurrOpenFileDialog.FileNames[0].Remove(0, CurrOpenFileDialog.FileNames[0].Length - 4) == ".csv")
            {
                ...
                {
                    // let's update the screening directory
                    for (int i = 1; i < CurrOpenFileDialog.FileNames.Length; i++)
                    {
                        string Tmp = CurrOpenFileDialog.FileNames[i].Remove(CurrOpenFileDialog.FileNames[i].Length - CurrOpenFileDialog.SafeFileNames[i].Length);
                        if (Tmp != cGlobalInfo.CurrentScreening.GetDirectory())
                        {
                            cGlobalInfo.CurrentScreening.SetDirectory("");
        ...
        private FormForImportExcel LoadCSVAssay(string[] FileNames, bool IsAppend)
        {
            ...
            if (FromExcel.ListPosPropertyOnGUI["Concentration"] != -1)
            {
                FromExcel.dataGridViewForImport.Rows[FromExcel.ListPosPropertyOnGUI["Concentration"]].Cells[2].Value = "Concentration";
            ...
            if (FromExcel.ListPosPropertyOnGUI["Group"] != -1)
            {
                FromExcel.dataGridViewForImport.Rows[FromExcel.ListPosPropertyOnGUI["Group"]].Cells[2].Value = "Group";
                FromExcel.dataGridViewForImport.Rows[FromExcel.ListPosPropertyOnGUI["Group"]].Cells[1].Value = true;
            }
        ...
        private void linkToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < ET.ListRowNames.Count; i++)
            {
                ...
                string CpdName = (string)(ET[0].ListTags[i]);


                for (int IdxWell = 1; IdxWell < ET.Count; IdxWell++)
                {

                    string WellPos = (string)(ET[IdxWell].ListTags[i]);
                    if (WellPos == null) continue;
                    WellPos = WellPos.Trim();


                    // first retrieve the plate
                    cListPlates TmpListPlatesActive = cGlobalInfo.CurrentScreening.ListPlatesActive;
                    foreach (cPlate TmpPlate in TmpListPlatesActive)
                    {
                        if (TmpPlate.GetName() != PlateName) continue;
