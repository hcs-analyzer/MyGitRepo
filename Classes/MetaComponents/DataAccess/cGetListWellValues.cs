...
namespace HCSAnalyzer.Classes.MetaComponents
{
    class cGetListWellValues : cComponent
    {
        ...
        public cGetListWellValues()
        {
            ...
            Prop1.Info = "Include Images Within the Data (if available).";
            Prop1.SetNewValue((bool)false);
            base.ListProperties.Add(Prop1);


            cPropertyType PT1 = new cPropertyType("Include Classes?", eDataType.BOOL);
            cProperty Prop2 = new cProperty(PT1, null);
            Prop2.Info = "Include Well Classes as a Column.";
            Prop2.SetNewValue((bool)false);
        ...
        cFeedBackMessage Process()
        {
            ...
            _firstValue = base.ListProperties.FindByName("Include Classes?");
            bool IsIncludeClasses = false;
            if (_firstValue == null)
            {
                base.GenerateError("-Include Classes?- not found !");
                return base.FeedBackMessage;
            }
            try
            {
                cProperty TmpProp = (cProperty)_firstValue;
                IsIncludeClasses = (bool)TmpProp.GetValue();
            }
            catch (Exception)
            {
                base.GenerateError("-Include Classes?- cast didn't work");
                return base.FeedBackMessage;
            }

            this.Output = this.Input.GetAverageDescriptorValues(cGlobalInfo.CurrentScreening.ListDescriptors.GetActiveDescriptors(), false, IsIncludeClasses);

            #region Include Images
            if (IsIncludeImages)
            {
