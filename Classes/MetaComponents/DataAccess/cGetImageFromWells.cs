...
namespace HCSAnalyzer.Classes.MetaComponents
{
    class cGetImageFromWells : cComponent
    {
        ...
        cFeedBackMessage Process()
        {
            ...
            if(ImageMetaInfo[0].NumZ==1)
             this.Output = new cImage(ImageMetaInfo);
            else
            {
                for (int Z = 0; Z < ImageMetaInfo[0].NumZ; Z++)
			    {
                    foreach (var item in ImageMetaInfo)
	                {
		                 item.PositionZ = Z;
	                }
                
			        cImage TmpIm =  new cImage(ImageMetaInfo); 
                    //this.Output =
			    } 
            }
            

            //this.Output = new cImage(10000, 3000, 1, 1);
