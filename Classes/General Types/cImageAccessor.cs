...
namespace HCSAnalyzer.Classes.General_Types
{
    ...
    public enum eImagingPlatformType { HARMONY, HARMONY35, IMAGEXPRESS, CELLOMICS, INCELL, CV7000, BUILTIN, MANUAL, KINETIC, NONE };
        ...
        public List<cImageMetaInfo> GetImageInfo(object Sender)
        {
            ...
            if (this.ImagingPlatformType == eImagingPlatformType.HARMONY)
            {
                ...
                if (Sender.GetType() == typeof(cSingleBiologicalObject))
                {
                    ...
                    for (int Channel = 1; Channel <= NumberOfChannels; Channel++)
                    {
                        ...
                        TmpInfo.Field = this.Field;

                        if((this!=null)&&(Sender.GetType()==typeof(cWell)))
                            TmpInfo.AssociatedWell = (cWell)Sender;
            ...
            if (this.ImagingPlatformType == eImagingPlatformType.IMAGEXPRESS)
            {
                if (Sender.GetType() == typeof(cSingleBiologicalObject))
                {
                    ...
                    for (int Channel = 1; Channel <= NumberOfChannels; Channel++)
                    {
                        ...
                        TmpMetaInfo.Field = this.Field;
                        if ((this != null) && (Sender.GetType() == typeof(cWell)))
                            TmpMetaInfo.AssociatedWell = (cWell)Sender;
            ...
            if (this.ImagingPlatformType == eImagingPlatformType.CELLOMICS)
            {
                ...
                else if (Sender.GetType() == typeof(cWell))
                {
                    ...
                    try
                    {
                       // TestDir = Directory.GetDirectories(InitialPath, WellObject.AssociatedPlate.GetName(), SearchOption.AllDirectories);
                        ListDir = Directory.EnumerateDirectories(InitialPath, "*"+ WellObject.AssociatedPlate.GetName(), SearchOption.AllDirectories);
                    }
                    catch (Exception e)
                    {
                    ...
                    try
                    {
                        string[] ListDBFiles = Directory.GetFiles(NewDir, "*.mdb");
                        if (ListDBFiles.Length > 1) return null; // more than one DB .......something is wrong.


                        String connectionString = @"Provider=Microsoft.JET.OlEDB.4.0;" + "Data Source=" + ListDBFiles[0];
                        ...
                        for (int Channel = 0; Channel < NumberOfChannels; Channel++)
                        {
                            ...
                            TmpMetaInfo.FileName = Directory.GetFiles(NewDir,"*" + Name)[0];
                            ListImageMetaInfo.Add(TmpMetaInfo);
                        }
            ...
            if (this.ImagingPlatformType == eImagingPlatformType.INCELL)
            {
                ...
                //IFW.SetInputData(new cListWells(WellObject));
                //IFW.ListProperties.FindByName("Field").SetNewValue(this.Field);
                //IFW.Run();
            ...
            if (this.ImagingPlatformType == eImagingPlatformType.CV7000)
            {
                ...
                if (File.Exists(XMLFile))
                {
                    ...
                    for (int Channel = 0; Channel < NumberOfChannels; Channel++)
                    {
                        ...
                        TmpMetaInfo.ResolutionZ = 1;
                        TmpMetaInfo.NumZ = int.Parse(xmlDoc.ChildNodes[1].Attributes["bts:ZCount"].Value);
            ...
            if (this.ImagingPlatformType == eImagingPlatformType.BUILTIN)
            {
                ...
                if (Sender.GetType() == typeof(cSingleBiologicalObject))
                {
                    ...
                    // cannot have thumbnail of a biological object
                }
                else if (Sender.GetType() == typeof(cWell))
                {
                    WellObject = (cWell)Sender;
               ...
               // string sField = (this.Field + 1).ToString("D2");
                string FinalPos = "*.p" + WellObject.AssociatedPlate.GetName() + ".w" + Pos.ToLower() + ".s" + (this.Field + 1).ToString("D2") + "*.*";
                ...
                {
                    FinalPos = "*.p" + WellObject.AssociatedPlate.GetName() + ".w" + Pos.ToUpper() + ".s" + (this.Field + 1).ToString("D2") + "_*.*";
                    ListChannels = Directory.GetFiles(InitialPath, FinalPos, SearchOption.TopDirectoryOnly);
                    NumberOfChannels = ListChannels.Count();
                }

                if(NumberOfChannels==0)
                {
                    FinalPos = "*.p" + WellObject.AssociatedPlate.GetName() + ".w" + Pos.ToUpper() + ".s" + (this.Field + 1).ToString("D1") + "_*.*";
                    ListChannels = Directory.GetFiles(InitialPath, FinalPos, SearchOption.TopDirectoryOnly);
                    NumberOfChannels = ListChannels.Count();
                }
                ...
                if (NumberOfChannels == 0)
                {
                    FinalPos = "*.p" + WellObject.AssociatedPlate.GetName() + ".w" + Pos.ToLower() + ".s" + (this.Field + 1).ToString("D1") + "_*.*";
                    ListChannels = Directory.GetFiles(InitialPath, FinalPos, SearchOption.TopDirectoryOnly);
                    NumberOfChannels = ListChannels.Count();
                }
            ...
            #endregion
            #region Kinetic
            if (this.ImagingPlatformType == eImagingPlatformType.KINETIC)
            {
                cWell WellObject = null;
                ...
                string FinalPos = InitialPath + "\\" + WellObject.AssociatedPlate.GetName() + "\\" + Pos.ToUpper() + "\\";

                if (!Directory.Exists(FinalPos)) return null;

                string[] ListMovies = Directory.GetFiles(FinalPos, "Original*_pre.avi", SearchOption.AllDirectories);
                System.Diagnostics.Process MyProcess = new System.Diagnostics.Process();
                MyProcess.StartInfo.FileName = ListMovies[0];
                MyProcess.Start();

                ListMovies = Directory.GetFiles(FinalPos, "Original*_post.avi", SearchOption.AllDirectories);
                MyProcess = new System.Diagnostics.Process();
                MyProcess.StartInfo.FileName = ListMovies[0];
                MyProcess.Start();

                ListImageMetaInfo = null;
