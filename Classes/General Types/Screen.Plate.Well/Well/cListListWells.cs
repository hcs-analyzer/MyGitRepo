    ...
    public class cListListWells : List<cListWells>
    {
        ...
        public void BuildAssociated3DDRC()
        {
           ...
           // GlobalList.Add(SpaceCube);
            ...
            legend.SetNumberOfEntries(this.Count);
            legend.BorderOn();
            ...
            foreach (cListWells SingleDRCListWells in this)
            {
                ...
                Color DRCColor = cGlobalInfo.ListGroupInfo.GetColor(IdxDRC);
                My3dDRC.Create(new cPoint3D(0, 0, 0), DRCColor);
                ...
                c3DSphere TmpSphere = new c3DSphere(My3dDRC.ListAveragePts[0] - My3dDRC.ListAveragePts[0], 0.030, Color.FromArgb(100, My3dDRC.Colour));
                TmpSphere.SetName(My3dDRC.GetName() + " - Starting Pt");
                SpheresList.AddObject(TmpSphere);
   
                
                // add the legend
                legend.SetEntryString(IdxDRC, My3dDRC.GetName());
                legend.SetEntryColor(IdxDRC, DRCColor.R / 255.0, DRCColor.G / 255.0, DRCColor.B / 255.0);
                
                IdxDRC++;
            }

            legend.GetActualPositionCoordinate().SetCoordinateSystemToView();
            legend.GetPositionCoordinate().SetValue(-1, -1.0);
            legend.GetPosition2Coordinate().SetCoordinateSystemToView();
            legend.GetPosition2Coordinate().SetValue(-.5, -0.5);
            ...
            MyWorld.AssociatedVTKRenderer.AddActor(legend);
                #endregion
        }
    }
}
