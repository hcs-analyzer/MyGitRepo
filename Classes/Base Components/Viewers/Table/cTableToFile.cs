            ...
            if (this.FileType == eFileType.CSV)
            {
                ...
                else
                {
                    using (StreamWriter myFile = new StreamWriter(FilePath, this.IsAppend, Encoding.Default))
                    {
                        ...
                        if ((this.Input.ListRowNames != null)&&(this.Input.ListRowNames.Count > 0))
                            sHeaders += this.Separator;
                        ...
                        // Export data.  
                        for (int i = 0; i < this.Input[0].ListTags.Count; i++)
                        {
                            string stLine = "";
                            ...
                            for (int j = 0; j < this.Input.Count; j++) { stLine = stLine.ToString() + this.Input[j].ListTags[i].ToString() + Separator; }
