...
namespace HCSAnalyzer.Classes.Base_Classes.DataStructures
{
    ...
    public class cExtendedTable : List<cExtendedList>
    {
        ...
        public double[][] CopyToTransposedArray2()
        {
            double[][] ToReturn = new double[this[0].Count][];

            for (int i = 0; i < this[0].Count; i++)
            {
                ToReturn[i] = new double[this.Count];
            }

            for (int Col = 0; Col < this[0].Count; Col++)
            {
        ...
        public void RemoveByName(List<string> ColumnNames)
        {
            for (int i = 0; i < ColumnNames.Count; i++)
            {
                for (int j = 0; j < this.Count; j++)
                {
                    if (this[j].Name == ColumnNames[i])
                        this.RemoveAt(j--);
                }
            }
        }
